...
export default function EmotionTheme() {
  ...
  const dispatchMood = (m: Mood) => {
    ...
    if (m === "cheerful" || m === "happy")
      root.classList.add("mood-happy", "mood-cheerful");
    ...
    const tagline =
      m === "happy" || m === "cheerful"
        ? "Your smile just lit up my universe 🌟."
        : m === "professional"
          ? "Focus mode: Let me show you what I’ve built 🚀."
          : m === "surprised"
            ? "Whoa! You just discovered surprise mode ✨."
            : m === "angry"
              ? "Taking a strong stance. Let’s build boldly 🔥."
              : m === "sad"
                ? "Calm mode engaged. Here’s what I’ve crafted 🤎."
                : undefined;
    window.dispatchEvent(
      new CustomEvent("mood-change", { detail: { mood: m, tagline } }),
    );
    // audio mood
    if (!audioRef.current) return;
    if (m === "happy" || m === "cheerful") {
      audioRef.current.src =
        "https://cdn.pixabay.com/download/audio/2022/03/10/audio_3a7e2a662a.mp3?filename=happy-day-112194.mp3";
      audioRef.current.play().catch(() => {});
    } else if (m === "professional" || m === "sad") {
      audioRef.current.src =
        "https://cdn.pixabay.com/download/audio/2021/09/01/audio_81e0d90b76.mp3?filename=ambient-piano-110241.mp3";
      audioRef.current.play().catch(() => {});
    } else if (m === "surprised" || m === "angry") {
      audioRef.current.src =
        "https://cdn.pixabay.com/download/audio/2022/03/15/audio_3fc7b0df56.mp3?filename=cinematic-whoosh-hit-14596.mp3";
      audioRef.current.play().catch(() => {});
    }
  };
  ...
  return (
    <div className="fixed bottom-6 left-6 z-40 flex items-center gap-2">
      <button className="btn-neon" onClick={() => setEnabled((v) => !v)}>
        {enabled ? "Emotion: On" : "Emotion Mode"}
      </button>
      <div className="glass p-2 rounded-xl flex items-center gap-2">
        ...
        <button
          ...
          onClick={() => setManual("happy")}
        >
          Happy
        </button>
        <button
          className="px-2 py-1 rounded bg-white/10 border border-white/20 text-xs"
          onClick={() => setManual("angry")}
        >
          Angry
        </button>
        <button
          className="px-2 py-1 rounded bg-white/10 border border-white/20 text-xs"
          onClick={() => setManual("sad")}
        >
          Sad
        </button>
      </div>
      ...
      {showConsent && (
        ...
        >
          <div
            className="glass neon-border p-6 rounded-2xl max-w-md"
            onClick={(e) => e.stopPropagation()}
          >
            <h3 className="font-semibold text-lg mb-2">Try Emotion Mode?</h3>
            <p className="text-sm text-foreground/80">
              Allow webcam to see how the portfolio adapts to your mood. No data
              is stored.
            </p>
